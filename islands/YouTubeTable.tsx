...
export default function YouTubeTable() {
  const [youtubes, setYoutubes] = useState<YouTubeVideo[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [loadedCount, setLoadedCount] = useState(0);
  const totalCount = arrs.length;

  // 快取讀取，會檢查過期與版本
  function getCache(): YouTubeVideo[] | null {
    try {
  ...
  function setCache(data: YouTubeVideo[]) {
  ...
  async function loadData() {
    ...
    const newYoutubes: YouTubeVideo[] = [];
    ...
    for (let start = 0; start < arrs.length; start += MAX_BATCH_SIZE) {
      ...
      try {
        ...
        data.items.forEach((videoItem: any) => {
          newYoutubes.push({
            items: videoItem,
  ...
  return (
    <div class="max-w-7xl mx-auto p-4">
      ...
      {isLoading ? (
        <div class="loading-container text-center text-lg font-medium mb-4 min-h-[200px] flex justify-center items-center">
